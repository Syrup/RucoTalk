...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  if (!session) return { isLoggedIn: false, user: null, expired: 0 };
  ...
  try {
    ...
    console.log(session.get("secret"), session.data);
    // client.setSession(cookieHeader!);
    const account = new Account(sessionClient);
    console.log(await account.get());
    const user = await account.get();

    return json({
      isLoggedIn: (await account.get()) ? true : false,
      user,
    });
  } catch (err) {
    return json({
